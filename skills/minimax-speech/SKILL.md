...
# 从文件读取文本
bun run ${CLAUDE_SKILL_DIR}/scripts/tts.ts --text-file script.txt --output output.mp3

# 从 stdin 读取文本（管道输入）
echo "欢迎使用" | bun run ${CLAUDE_SKILL_DIR}/scripts/tts.ts --text-file - -o welcome.mp3
```
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `text` | string | 必填* | 要合成的文本（或改用 `--text-file`） |
| `--text-file`, `-f` | string | 无 | 从文件读取文本，使用 `-` 从 stdin 读取 |
...
| `--subtitle` | boolean | `false` | 同时生成字幕文件（JSON + SRT） |
...
Agent 决策：
- 音色：male-qn-jingying（精英男声，适合教程）
- 语速：1.0（正常）
- 字幕：开启
- 命令：bun run ${CLAUDE_SKILL_DIR}/scripts/tts.ts "教程内容..." --voice male-qn-jingying --subtitle --output tutorial.mp3

输出文件：
- tutorial.mp3（音频）
- tutorial_subtitle.json（字幕 JSON）
- tutorial_subtitle.srt（字幕 SRT，可直接导入剪辑软件）
```
