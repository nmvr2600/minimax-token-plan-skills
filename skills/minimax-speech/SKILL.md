...
name: minimax-speech
description: |
  当用户需要将文字转为语音、生成音频文件、语音合成、text to speech、TTS，或说"把这段文字转成语音"、
  "生成一个音频"、"读一下这段文字"时触发。

  **Agent 职责**：根据用户描述智能选择音色、语速等参数，不要询问用户，直接选择最合适的。
---

# MiniMax Speech Synthesis

MiniMax 异步语音合成 API，支持将文本转换为高质量语音音频。
...
```bash
# 基础用法 - Agent 已选好音色
python scripts/text_to_speech.py "文本内容" --voice female-tianmei --speed 1.0 --output welcome_message.mp3

# 完整参数
python scripts/text_to_speech.py "文本" \
...
output = text_to_speech(
    text="要合成的文本",
    voice_id="female-tianmei",  # Agent 根据场景选择
    speed=1.0,                   # Agent 根据需求选择
    output_file="story_chapter_1.mp3"
)
```
...
| `--voice`, `-v` | string | `female-tianmei` | 音色ID |
| `--speed`, `-s` | float | `1.0` | 语速 0.5-2.0 |
| `--vol` | int | `10` | 音量 0-10 |
| `--pitch`, `-p` | float | `1.0` | 音调 0.5-2.0 |
...
- 语速：1.0（正常）
- 命令：python scripts/text_to_speech.py "欢迎使用我们的服务" --voice female-tianmei --output welcome.mp3
```
...
- 语速：1.0（标准）
- 命令：python scripts/text_to_speech.py "[新闻文本]" --voice "Chinese (Mandarin)_News_Anchor" --output news_broadcast.mp3
```
...
- 语速：0.9（稍慢，更有氛围）
- 命令：python scripts/text_to_speech.py "[故事文本]" --voice female-yujie --speed 0.9 --output story_chapter_1.mp3
```
...
- 语速：1.2（稍快）
- 命令：python scripts/text_to_speech.py "紧急通知：请立即疏散" --voice male-qn-qingse --speed 1.2 --output emergency_notice.mp3
```
