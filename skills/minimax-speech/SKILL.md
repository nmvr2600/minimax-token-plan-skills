...
```bash
# 基础用法 - Agent 已选好音色
bun run skills/minimax-speech/scripts/tts.ts "文本内容" --voice female-tianmei --speed 1.0 --output welcome_message.mp3

# 或使用 package.json 中的快捷命令
bun run tts "文本内容" --voice female-tianmei --speed 1.0 --output welcome_message.mp3

# 完整参数
bun run skills/minimax-speech/scripts/tts.ts "文本" \
...
- 语速：1.0（正常）
- 命令：bun run scripts/tts.ts "欢迎使用我们的服务" --voice female-tianmei --output welcome.mp3
```
...
- 语速：1.0（标准）
- 命令：bun run scripts/tts.ts "[新闻文本]" --voice "Chinese (Mandarin)_News_Anchor" --output news_broadcast.mp3
```
...
- 语速：0.9（稍慢，更有氛围）
- 命令：bun run scripts/tts.ts "[故事文本]" --voice female-yujie --speed 0.9 --output story_chapter_1.mp3
```
...
- 语速：1.2（稍快）
- 命令：bun run scripts/tts.ts "紧急通知：请立即疏散" --voice male-qn-qingse --speed 1.2 --output emergency_notice.mp3
```
