...
| 任务 | 命令 |
|------|------|
| 自动分析图片 | `bun run scripts/analyze.ts "image.jpg"` |
| 指定分析要求 | `bun run scripts/analyze.ts "image.jpg" "提取所有文字"` |
...
```bash
bun run scripts/analyze.ts "document.jpg" "提取图中所有文字，保留原有格式"
```

### 场景二：理解截图内容

```bash
bun run scripts/analyze.ts "screenshot.png" "描述这个截图显示的是什么界面，有什么功能"
```

### 场景三：分析图表数据

```bash
bun run scripts/analyze.ts "chart.png" "分析这个柱状图的主要数据趋势和结论"
```

### 场景四：识别物体和场景

```bash
bun run scripts/analyze.ts "photo.jpg" "识别图片中的物体、人物动作和拍摄场景"
```

### 场景五：代码截图分析

```bash
bun run scripts/analyze.ts "code.png" "提取图中的代码并解释其功能"
```
...
```bash
# 提示词过于模糊
bun run scripts/analyze.ts "image.jpg" "看一下"
```
...
```bash
# 提示词具体明确
bun run scripts/analyze.ts "image.jpg" "提取图中所有的文字内容，包括标题、正文和按钮标签"
```
